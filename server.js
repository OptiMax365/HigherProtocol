...
app.use(cors({ origin: CORS_ORIGIN, credentials: true, methods: ['GET','POST','PUT','DELETE'] }));
...
app.post('/api/signup', authLimiter, async (req, res) => {
  try {
    ...
    // Return consistent field naming for isPro
    res.json({ 
      token, 
      user: { 
        email: user.email, 
...
// LOGIN - Fixed to return consistent isPro field
...
app.get('/api/me', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT email, name, is_pro as "isPro", plan, paid_at FROM users WHERE email = $1',
      [req.user.email]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: 'User not found' });
    const user = result.rows[0];
    // Ensure isPro is boolean
    res.json({ 
      email: user.email, 
      name: user.name, 
      isPro: !!user.isPro, 
  ...
  try {
    ...
    if (process.env.SMTP_USER) {
      await transporter.sendMail({
        ...
        html: `<h3>Password Reset Request</h3><p><strong>Email:</strong> ${email}</p><p><strong>IP:</strong> ${ip}</p><p><strong>Time:</strong> ${new Date().toLocaleString()}</p><p>Log into admin dashboard to send recovery email.</p>`
      });
    }
  ...
  try {
    ...
    if (process.env.SMTP_USER) {
      await transporter.sendMail({
        ...
        html: `<h3>Reset Your Password</h3><p>Hi ${user.rows[0].name},</p><p>Click the link below to reset your password:</p><p><a href="${resetLink}" style="background:#6e3bfa;color:#fff;padding:10px 20px;text-decoration:none;border-radius:6px">Reset Password</a></p><p>This link expires in 1 hour.</p><p>Requested by admin: ${adminEmail}</p>`
      });
    }
...
// === PING ENDPOINT for activity tracking ===
