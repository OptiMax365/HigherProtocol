...
async function initDB() {
  try {
    ...
    if (adminCheck.rows.length === 0) {
      ...
      console.log(`✅ Admin user created: ${ADMIN_EMAIL} with password: ${ADMIN_PASSWORD}`);
    } else {
...
res.status(201).json({ token, user }); // Explicit 201 status
} catch (err) {
...
app.post('/api/login', authLimiter, async (req, res) => {
  try {
    ...
    res.json({
      token,
      user: { email: user.email, name: user.name, isPro: user.is_pro, plan: user.plan }
    });
  } catch (err) {
...
// Add to server.js - Track real-time presence
